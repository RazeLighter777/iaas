...
nix store sign --store "s3://${NIX_S3_BUCKET}?endpoint=${NIX_S3_ENDPOINT}&region=${NIX_S3_REGION}&scheme=${NIX_S3_SCHEME:-https}&profile=${AWS_PROFILE:-default}" --key-file "${NIX_CACHE_PRIVATE_KEY_FILE}" ${paths}